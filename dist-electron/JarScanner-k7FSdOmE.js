import { g as getDefaultExportFromCjs, f as fs } from "./index-D3XxUJRe.js";
...
Utils$5.prototype.writeFileTo = function(path, content, overwrite, attr) {
  const self = this;
...
Utils$5.prototype.writeFileToAsync = function(path, content, overwrite, attr, callback) {
...
Utils$5.prototype.findFiles = function(path) {
  const self = this;
  function findSync(dir, pattern, recursive) {
    let files = [];
    self.fs.readdirSync(dir).forEach(function(file) {
      const path2 = pth$2.join(dir, file);
      const stat = self.fs.statSync(path2);
      {
        files.push(pth$2.normalize(path2) + (stat.isDirectory() ? self.sep : ""));
      }
...
var fattr = function(path, { fs: fs2 }) {
...
class JarScanner {
  ...
  static async scanDirectory(dirPath) {
    ...
    for (const file of files) {
      const fullPath = require$$1.join(dirPath, file);
