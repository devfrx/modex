...
import { g as getDefaultExportFromCjs, f as fs$1 } from "./index-D3XxUJRe.js";
...
class ModDatabase {
  constructor() {
    ...
    const dbPath = require$$1.join(userDataPath, "modex.db");
    fs$1.ensureDirSync(require$$1.dirname(dbPath));
  ...
  init() {
    ...
    `);
    this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_mods_loader ON mods(loader);
      CREATE INDEX IF NOT EXISTS idx_mods_game_version ON mods(game_version);
      CREATE INDEX IF NOT EXISTS idx_modpack_mods_modpack ON modpack_mods(modpack_id);
      CREATE INDEX IF NOT EXISTS idx_modpack_mods_mod ON modpack_mods(mod_id);
    `);
  }
  // ========== MODS CRUD ==========
  getAllMods() {
    const stmt = this.db.prepare("SELECT * FROM mods ORDER BY name ASC");
    return stmt.all();
  }
  getModById(id) {
  ...
  addMod(mod) {
    const stmt = this.db.prepare(`
      INSERT INTO mods (filename, name, version, game_version, loader, description, author, path, hash)
      VALUES (@filename, @name, @version, @game_version, @loader, @description, @author, @path, @hash)
    `);
    const result = stmt.run(mod);
    return result.lastInsertRowid;
  }
  updateMod(id, mod) {
    const fields = Object.keys(mod).filter(
      (k) => k !== "id" && k !== "created_at"
    );
    const setClause = fields.map((f) => `${f} = @${f}`).join(", ");
    const stmt = this.db.prepare(`UPDATE mods SET ${setClause} WHERE id = @id`);
    stmt.run({ ...mod, id });
  }
  deleteMod(id) {
  ...
  // ========== MODPACKS CRUD ==========
  getAllModpacks() {
    const stmt = this.db.prepare("SELECT * FROM modpacks ORDER BY name ASC");
    return stmt.all();
  }
  getModpackById(id) {
    const stmt = this.db.prepare("SELECT * FROM modpacks WHERE id = ?");
    return stmt.get(id);
  }
  addModpack(modpack) {
    const stmt = this.db.prepare(`
      INSERT INTO modpacks (name, version, description)
      VALUES (@name, @version, @description)
    `);
    const result = stmt.run(modpack);
    return result.lastInsertRowid;
  }
  updateModpack(id, modpack) {
    const fields = Object.keys(modpack).filter(
      (k) => k !== "id" && k !== "created_at"
    );
    const setClause = fields.map((f) => `${f} = @${f}`).join(", ");
    const stmt = this.db.prepare(
      `UPDATE modpacks SET ${setClause} WHERE id = @id`
    );
    stmt.run({ ...modpack, id });
  }
  deleteModpack(id) {
    const stmt = this.db.prepare("DELETE FROM modpacks WHERE id = ?");
    stmt.run(id);
  }
  // ========== MODPACK-MOD RELATIONS ==========
  getModsInModpack(modpackId) {
    const stmt = this.db.prepare(`
      SELECT m.* FROM mods m
      INNER JOIN modpack_mods mm ON m.id = mm.mod_id
      WHERE mm.modpack_id = ?
      ORDER BY m.name ASC
    `);
    return stmt.all(modpackId);
  }
  addModToModpack(modpackId, modId) {
    const stmt = this.db.prepare(`
      INSERT OR IGNORE INTO modpack_mods (modpack_id, mod_id)
      VALUES (?, ?)
    `);
    stmt.run(modpackId, modId);
  }
  removeModFromModpack(modpackId, modId) {
    const stmt = this.db.prepare(`
      DELETE FROM modpack_mods WHERE modpack_id = ? AND mod_id = ?
    `);
    stmt.run(modpackId, modId);
  }
  getModpackCount(modId) {
    const stmt = this.db.prepare(`
      SELECT COUNT(*) as count FROM modpack_mods WHERE mod_id = ?
    `);
    const result = stmt.get(modId);
    return result.count;
  }
  close() {
    this.db.close();
