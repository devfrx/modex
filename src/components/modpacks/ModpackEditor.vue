...
function sanitizeRemoteUrl() {
  ...
  if (match) {
    ...
    if (newUrl !== editForm.value.remote_url) {
      ...
      toast.info("URL Sanitized", "Removed commit hash from Gist URL to ensure updates work.");
    }
  }
}

// Watch for remote URL changes to sanitize automatically
watch(() => editForm.value.remote_url, () => {
  sanitizeRemoteUrl();
});
...
async function exportManifest() {
  ...
  try {
    ...
    toast.success("Manifest Copied", "JSON copied to clipboard. You can now paste it into a GitHub Gist.");
  } catch (err) {
  ...
  try {
    ...
    if (result.hasUpdate && result.changes) {
      ...
      result.changes.addedMods.forEach((mod: any) => {
        changes.push({ type: 'add', modId: mod.name, modName: mod.name, newVersion: mod.version });
      });
    ...
    <div
      ...
      <div class="shrink-0 border-b border-border/50 bg-gradient-to-r from-card/80 to-card/40">
        ...
        <div v-if="updateResult?.hasUpdate" class="px-5 pb-4">
          <UpdateAvailableBanner :current-version="modpack?.version || 'unknown'"
            :new-version="updateResult.remoteManifest?.modpack.version || 'unknown'" :is-checking="isCheckingUpdate"
