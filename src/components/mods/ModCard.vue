...
<template>
  <div
    ...
    <div class="p-4 relative z-10 flex flex-col h-full">
      ...
      <div class="flex items-start gap-3 mb-3">
        <div
          ...
          <div v-else class="w-full h-full flex items-center justify-center">
            <Box class="w-5 h-5 text-muted-foreground/50 group-hover:text-primary/50 transition-colors" />
          </div>
        </div>
      ...
      <!-- Description -->
      <p class="text-xs text-muted-foreground/80 line-clamp-2 mb-3 flex-1 min-h-[2rem] leading-relaxed">
      ...
      <div class="space-y-2 mt-auto">
        <!-- Categories -->
        <div v-if="mod.categories && mod.categories.length" class="flex flex-wrap gap-1 h-4 overflow-hidden">
          <span v-for="category in mod.categories.slice(0, 2)" :key="category"
            class="text-[9px] uppercase tracking-wider font-medium px-1 py-0 rounded bg-secondary/50 text-secondary-foreground/70">
            {{ category }}
          </span>
          <span v-if="mod.categories.length > 2"
            class="text-[9px] uppercase tracking-wider font-medium px-1 py-0 rounded bg-secondary/50 text-secondary-foreground/50">
        ...
        <div class="flex items-center gap-1 flex-wrap">
          <!-- Content Type Badge -->
          <span class="px-1.5 py-0.5 rounded text-[9px] font-medium border flex items-center gap-0.5"
            :class="typeConfig.class">
            <component :is="typeConfig.icon" class="w-2.5 h-2.5" />
            {{ typeConfig.label }}
          </span>
          <!-- Loader (only for mods) -->
          <span v-if="contentType === 'mod'" class="px-1.5 py-0.5 rounded text-[9px] font-medium" :class="mod.loader?.toLowerCase().includes('forge')
        ...
        <div class="flex items-center justify-between gap-2 pt-2 border-t border-white/5">
          ...
          <div class="flex items-center gap-1">
            <button v-if="groupVariantCount && groupVariantCount > 0" @click.stop="$emit('toggle-group')"
              class="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold transition-all" :class="isGroupExpanded
            ...
            <div v-else-if="isVariant"
              class="flex items-center gap-1 px-2 py-1 rounded-md bg-primary/10 text-primary text-[10px] font-medium">
          ...
          <div class="flex items-center gap-0.5 shrink-0" @click.stop>
            ...
            <Button variant="ghost" size="icon"
              class="h-6 w-6 text-muted-foreground/70 hover:text-destructive hover:bg-destructive/10"
